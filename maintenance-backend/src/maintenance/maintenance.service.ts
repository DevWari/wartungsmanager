...
import { DataResponse, MOCK_AUTH_INFO_FOR_DEV } from 'shared/nestjs';
...
export class MaintenanceService {
  ...
  constructor(
    ...
    this.checkStatusesEveryDayForNewIntervals();
  }
  ...
  async completeMaintenance(id: string): Promise<Maintenance> {
    ...
    if (maintenance && !maintenance.copied) {
      const { id, createdAt, status, ...rest } = maintenance;

      let copy = null;
      if (maintenance.useOperatingHours || maintenance.useStrokes || maintenance.useDistance) {
        copy = {
          ...rest,
          dueAt: maintenance.dueAt * 2,
          earliestProcessing: maintenance.earliestProcessing + maintenance.dueAt,
        } as Maintenance;
      } else if (
  ...
  private async checkStatusesEveryDay() {
    ...
    let idsToUpdate: string[] = (
      await this.maintenanceRepo
        ...
        .andWhere('use_operating_hours = :value', {value: 0})
        .andWhere('use_distance = :value', {value: 0})
        .andWhere('use_strokes = :value', {value: 0})
        .getRawMany()
    ).map(({ id }: { id: string }) => id);
  ...
  @Cron('*/15 * * * *')
  private async checkStatusesEveryDayForNewIntervals() {
    this.logger.log('Running cron job. Updating maintenances...');

    let maintenances = (
      await this.maintenanceRepo
        .createQueryBuilder()
        .select('id, machine_id, tenant_id, use_operating_hours, use_distance, use_strokes, earliest_processing, due_at')
        .andWhere('status <> "completed"')
        .andWhere('status <> "overdue"')
        .andWhere(new Brackets(qb => {
          qb.where('use_operating_hours = :value', {value: 1})
            .orWhere('use_distance = :value', {value: 1})
            .orWhere('use_strokes = :value', {value: 1})
        }))
        .getRawMany()
    );

    let idsToUpdateForDueSoon: string[] = [];
    let idsToUpdateForOverdue: string[] = [];
    for(let i=0; i<maintenances.length; i++) {
      const {id, machine_id, tenant_id, use_operating_hours, use_distance, use_strokes, earliest_processing, due_at} = maintenances[i];
      const opts = this.configService.getRequestHeader({
        ...MOCK_AUTH_INFO_FOR_DEV,
        tenantId: tenant_id
      });
      const properties = (
        await this.httpService
          .get<DataResponse<UnitedPropertyDto[]>>(
            `${this.serviceUrl}/v1/properties/asset/${machine_id}`,
            opts,
          )
          .toPromise()
      ).data.data;
      ...
      let index = -1;
      if (use_operating_hours === 1) {
        index = properties.findIndex(prop => prop.key === OPERATING_HOURS_PROP_KEY);
      } else if (use_distance === 1) {
        index = properties.findIndex(prop => prop.key === DISTANCE_PROP_KEY);
      } else {
        index = properties.findIndex(prop => prop.key === STROKES_PROP_KEY);
      }

      const propertyValue = index > -1 ? Number(properties[index].value) : 0;
      if (propertyValue >= due_at) {
        idsToUpdateForOverdue.push(id);
      } else if (propertyValue >= earliest_processing) {
        idsToUpdateForDueSoon.push(id);
      } else {}
    }

    if (idsToUpdateForOverdue.length > 0) {
      this.logger.log(`Changing ${idsToUpdateForOverdue.length} maintenances to status [overdue]...`);
      await this.maintenanceRepo.update(idsToUpdateForOverdue, { status: 'overdue' });
      const updatedMaintenances = await this.maintenanceRepo.findByIds(idsToUpdateForOverdue);
      await Promise.all(
        updatedMaintenances.map(async (m: Maintenance) => {
          const maintenance = (await this.getMaintenance(m.id)) as Maintenance;
          await this.copyMaintenance(
    ...
    if (idsToUpdateForDueSoon.length > 0) {
      this.logger.log(`Changing ${idsToUpdateForDueSoon.length} maintenances to status [dueSoon]...`);
      await this.maintenanceRepo.update(idsToUpdateForDueSoon, { status: 'dueSoon' });
    }
  }
